...
import dynamicImport from 'next/dynamic'; // ✅ เปลี่ยนชื่อ
...
export default function Home() {
  ...
  const displayLogs =
    selectedDeviceMac === 'ALL'
      ? logs
      : logs.filter(l =>
          normalize(l.mac || l.device_id) === normalize(selectedDeviceMac)
        );
  ...
  const currentLevel = Number(
    latestLog?.water_level ??
    latestLog?.level ??
    currentDevice?.waterLevel ??
    0
  );
  ...
  const currentHumid = Number(
    latestLog?.humidity ??
    latestLog?.air_humidity ??
    ...
    <div className="min-h-screen bg-slate-50 p-4 md:p-8 font-sans text-slate-900">
      ...
      <header className="max-w-7xl mx-auto flex justify-between items-center mb-8">
        ...
        <div className="flex items-center gap-4">
          ...
          <div className="relative">
            <button 
              ...
              className="flex items-center gap-2 bg-white px-4 py-2 rounded-xl shadow-sm border font-bold text-sm uppercase"
            >
              {selectedDeviceMac === 'ALL'
                ? '🌍 Overview'
            ...
            {isDropdownOpen && (
              <div className="absolute top-full left-0 mt-2 w-56 bg-white shadow-2xl rounded-2xl border z-50 p-2">

                <button
                  onClick={() => {
                    setSelectedDeviceMac('ALL');
                    setIsDropdownOpen(false);
                  }}
                  className="w-full text-left px-4 py-2 hover:bg-slate-50 rounded-lg text-xs font-bold uppercase"
                >
                  🌍 Overview
                </button>
        ...
        <div className="flex items-center gap-3">
          ...
          <Link
            href="/admin"
            className="px-4 py-2 bg-slate-900 text-white rounded-xl text-xs font-bold uppercase shadow-lg flex items-center gap-2"
          >
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
          ...
          <div className="lg:col-span-8 bg-white p-6 rounded-[2.5rem] border shadow-sm h-[450px] flex flex-col">
                ...
                {['day', 'week', 'month'].map(tf => (
                  <button
                    ...
                    className={`px-3 py-1 text-[10px] font-bold uppercase rounded-lg ${
                      timeframe === tf ? 'bg-white shadow-sm' : 'text-slate-500'
                    }`}
                  >
                    {tf}
                  </button>
            ...
            <div className="flex-grow">
              <WaterLevelChart
                ...
                devices={devices} // ✅ เพิ่ม
                selectedDeviceMac={selectedDeviceMac} // ✅ เพิ่ม
              />
            </div>
          ...
          {/* DONUT */}
          <div className="lg:col-span-4 bg-white p-6 rounded-[2.5rem] border shadow-sm flex flex-col items-center justify-center">

            <h3 className="text-xs font-black uppercase tracking-widest mb-4 self-start">
              Status Distribution
            </h3>
        ...
        {/* MAP */}
        <div className="bg-white p-2 rounded-[2.5rem] border shadow-sm overflow-hidden">
