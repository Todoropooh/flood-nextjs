...
export async function GET(request: NextRequest) {
  try {
    ...
    // 🌟 ดึงข้อมูลทั้งหมดในช่วงเวลานั้น ไม่จำกัด Limit เพื่อให้วาดกราฟได้ครบ
...
export async function POST(request: NextRequest) {
  try {
    ...
    await Device.findOneAndUpdate({ mac }, { 
      waterLevel: wl, temperature: Number(temperature) || 0,
      humidity: Number(humidity) || 0, lastPing: new Date(), status: status
    });
